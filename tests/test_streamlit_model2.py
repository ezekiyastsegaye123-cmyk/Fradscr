...
# If executed directly by Streamlit Cloud or local Streamlit runner (e.g. if the entrypoint
# in Streamlit Cloud settings is set to this file), transparently redirect to streamlit_app.py.
_is_streamlit_run = (
    "streamlit.runtime" in sys.modules
...
if _is_streamlit_run:
    ...
    with open(app_file, "r", encoding="utf-8") as _f:
        _code = compile(_f.read(), str(app_file), "exec")
        exec(_code, globals())
else:
    try:
        import pytest
